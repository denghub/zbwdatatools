...
namespace bw323DataTools.Core
{
    public class DBToExcel
    {
        ...
        public static int ExportExceldbg(string TabelNameArray, string filename)
        {
            if (filename != null)
            {
                Stopwatch watch = Stopwatch.StartNew();
        ...
        public static async Task<int> ExportExcelAsync2(string TabelNameArray, string filename)
        {

            return await Task.Run(
                () => { return ExportExceldbg(TabelNameArray, filename); }
                );
